from PyQt5.QtWidgets import (
    ...
    QVBoxLayout, QMessageBox, QFormLayout, QDialogButtonBox, QFrame, QToolButton
)
...
class LoginDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        heading = QLabel("EDR-CONTINUOUS INFORMATION PVT. LTD.")
        ...
        self.project_dropdown.addItems(["Select a project"] + list(self.projects.keys()))
        form_layout.addRow("Select Project:", self.project_dropdown)
    ...
    def validate_login(self):
        ...
        if project_name == "Select a project":
            QMessageBox.warning(self, "Error", "Please select a valid project.")
            return
        ...
        if emp_id.lower() == "postgres" or emp_id == "17224":
            ...
            conn = self.connect_to_db(project_name, emp_id, password)
            if conn:
                success = self.load_editable_layer(self.designation, project_name, emp_id, password)
                if success:
        ...
        if not match.empty:
            ...
            if conn:
                if self.designation in EDITABLE_FIELDS:
                    ...
                    success = self.load_readonly_layer(project_name, emp_id, password)
                if success:
                    QMessageBox.information(self, "Success", "Login and layer loading successful!")
                    self.login_successful.emit()
                    self.reset_form()
    ...
    def connect_to_db(self, selected_db, username, password):
        config = self.projects[selected_db]
    ...
    def load_editable_layer(self, designation, selected_db, username, password):
        ...
        layer = QgsVectorLayer(uri, "Production Input (Editable)", "postgres")
        ...
        for emp_id_field, name_field in self.EMP_ID_TO_NAME_FIELD.items():
            if emp_id_field in attr_map:
                ...
                try:
                    ...
                    continue
                # Only update if name field is empty and emp_id is set
                ...
                def fetch_and_update(emp_id=employee_id, name_field=name_field, fid=fid):
                    ...
                    cursor = None
                    try:
                        config = self.projects[self.selected_project]
                        ...
                        cursor = conn.cursor()
                        cursor.execute(
                            "SELECT employee_name FROM public.employee WHERE employee_id = %s",
                            (emp_id,)
                        )
    ...
    def load_readonly_layer(self, selected_db, username, password):
        try:
            config = self.projects[selected_db]
            uri = (
                f"dbname='{config['dbname']}' host={config['host']} port={config['port']} "
            ...
            layer = QgsVectorLayer(uri, "Users Views (Read Only)", "postgres")
    ...
    def on_layers_removed(self, layer_ids):
        try:
            if self.current_layer and not sip.isdeleted(self.current_layer):
                if not QgsProject.instance().mapLayers(self.current_layer.id()):
                    if self.conn:
                        self.conn.close()
                        self.conn = None
                    self.reset_form()  # Clear form fields
    ...
    def fetch_employee_name(self, emp_id):
        ...
        try:
            ...
            logger.error("Error fetching employee name for %s: %s", emp_id, e)
